    ...
    if self.params.get("style_embedding_enable", False):
      ...
      with tf.variable_scope("style_encoder"):
        if ( self._model.get_data_layer().params.get("style_input", None) == "wav" ):
          ...
          if ( (self._mode=="infer" and self.params["use_saved_embedding"])):
          ...
          elif self._mode=="train_with_embeddings":
            style_embedding = tf.reshape( input_dict['source_tensors'][4], (tf.shape(embedded_inputs)[0], 512))
          else:
