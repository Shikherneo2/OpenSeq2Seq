...
def main():
	...
	if args.mode == 'train' or args.mode == 'train_eval' or args.benchmark:
		if hvd is None or hvd.rank() == 0:
			if checkpoint is None or args.benchmark:
				...
				deco_print( "Restored checkpoint from {}. Resuming training".format(checkpoint), )
	...
	with tf.Graph().as_default():
		model = create_model( args, base_config, config_module, base_model, hvd, checkpoint )
		hooks = None
		...
		if args.mode == "train_eval":
			train( model[0], eval_model=model[1], debug_port=args.debug_port, custom_hooks=hooks )
		
		elif args.mode == "train":
			train( model, eval_model=None, debug_port=args.debug_port, custom_hooks=hooks )
